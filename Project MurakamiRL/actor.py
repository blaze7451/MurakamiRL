...
class SoftmaxCategoricalHead(nn.Module):
    ...
    def forward(self, logits, temperature = 0.3):
        ...
        return Categorical(probs=softmax(logits/temperature))

class TextPPO(pfrl.agents.PPO):
    @autocast("cuda")
    def _update_if_dataset_is_ready(self):
        dataset_size = (
            sum(len(epsiode) for epsiode in self.memory)
            + len(self.last_episode)
            +(0 if self.batch_last_epsiode is None
            else sum(len(epsiode) for epsiode in self.batch_last_episode))
        )
...
class TextRLActor:
    ...
    def agent_ppo(self, update_interval=10, minibatch_size=3000, epochs=20):
        policy = nn.Sequential(self.converter, SoftmaxCategoricalHead())

        vf = nn.Sequential(nn.Linear(self.obs_size, 1))
